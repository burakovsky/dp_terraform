...
module "elb_web" {
  ...
  environment           = "${var.environment}"
  subnet_id             = ["${module.public_part.public_subnets}"]
  security_group_ids    = ["${module.sg_elb.aws_security_group}"]
...
# Create instances for services proxy servers
module "ec2_httpd" {
  source             = "./modules/ec2_inst"
  instance_count     = "${var.httpd_instance_count}"
  ami                = "${var.httpd_ami}"
  project_name       = "${var.project_name}"
  environment        = "${var.environment}"
  key_name           = "${module.key_pair.ec2_key_pair}"
  instance_type      = "${var.httpd_instance_type_ec2}"
  subnet_id          = "${module.private_part.private_subnets}"
  az                 = "${var.private_az}"
  pub_ip_bool        = "false"
  security_group_ids = ["${module.sg_ec2_vpc.aws_security_group}"]
...
module "elb_elk" {
  ...
  instance_id           = "${module.ec2_httpd.instance_id}"
